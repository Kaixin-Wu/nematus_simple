...
"""
    a' = g/\delta * (a - \mu) + b, g and b are gain parameters, \delta is variance, \mu is mean
"""
# layer normalization
...
def fflayer(tparams, state_below, options, dropout, prefix='rconv',
    ...
    # re-parameter `W` first, then apply dropout and do linear transformation
    if options['weight_normalisation'] and not followed_by_softmax:
         W = weight_norm(W, tparams[pp(prefix, 'W_wns')])
    preact = tensor.dot(state_below*dropout_mask, W) + b

    # before activation function, we need layer normalization first
    if options['layer_normalisation'] and not followed_by_softmax:
...
"""
    commnet: gru is a recurrent function -> h_i = gru(x_i, h_i-1)
    GRU:
        r_i = sigmod(Wr*x_i + Ur*h_i-1 + br)
        z_i = sigmod(Wz*x_i + Uz*h_i-1 + bz)
        h_i~ = tanh(W*x_i + U*(r_i*h_i-1) + b)
        note, in raw paper: Learning Phrase Representations using RNN Encoder-Decoder for Statistical Machine Translation
        h_i~ don't contain 'b', h_i~ = tanh(W*x_i + U*(r_i*h_i-1))
        h_i = z_i*h_i-1 + (1-z_i)*h_i~

    for gpu calculation, we put 'Wr' & 'Wz' together, as 'W'(in code)
                            we put 'Ur' & 'Uz' together, as 'U'(in code)
                            we put 'br' & 'bz' together, as 'b'(in code)
                            'W' is 'Wx'(in code), 'U' is 'Ux'(in code), 'b' is 'bx'(in code)
    so, 'W'(in code) shape is [embedding_size, 2*hidden_size]
        'U'(in code) shape is [hidden_size, 2*hidden_size]
        'b'(in code) shape is [2*hidden_size]
        'Wx'(in code) shape is [embedding_size, hidden_size]
        'Ux'(in code) shape is [hidden_size, hidden_size]
        'bx'(in code) shape is [hidden_size]
"""
# GRU layer
