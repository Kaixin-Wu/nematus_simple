    ...
    if options['enc_depth'] > 1:
        for level in range(2, options['enc_depth'] + 1):
            ...
            if level <= options['enc_depth_bidirectional']:
                ...
                params = get_layer_param(options['encoder'])(options, params,
                                                             prefix=prefix_f,
                                                             nin=options['dim'] * 2, # note input-dim is 2*hidden_size,
                                                             dim=options['dim'] * 2, # note output-dim is 2*hidden_size
                                                             recurrence_transition_depth=options['enc_recurrence_transition_depth'])
...
def build_encoder(tparams, options, dropout, x_mask=None, sampling=False):
    # x shape: factor_num * src_len * batch_size
...
# build a sampler
def build_sampler(tparams, options, use_noise, trng, return_alignment=False, return_identifier=False):
...
def gen_sample(f_init, f_next, x, trng=None, k=1, maxlen=30,
               ...
               return_hyp_graph=False, return_identifier=False):
    ...
    for i in xrange(num_models):
        ...
        ctx0[i] = ret[1]
        # we don't want to ensemble multi-model to get identifier, only one is OK.
        # so we only return the last model's identifier result
        if return_identifier:
            identifier = ret[2]
    ...
    return sample, sample_score, sample_word_probs, alignment, hyp_graph, identifier
...
def train(dim_word=512,  # word vector dimensionality
    ...
    if tie_encoder_decoder_embeddings:
        ...
        if worddicts[0] != worddicts[1]:
            warnings.warn("Encoder-decoder embedding tying is enabled with different source and target dictionaries. This is usually a configuration error")
    ...
    best_p = None # best model parameters
    best_opt_p = None # best optimizer parameters
...
######## MODEL ##############
    ...
    if reload_ and os.path.exists(saveto):
        print 'Reloading model parameters from %s' %(saveto)
    ...
    """
        ...
        the cost is a matrix [tgt_len, batch_size], which is word-level cost
    """
    trng, use_noise, \
        x, x_mask, y, y_mask, \
        opt_ret, \
    ...
    for training_progress.eidx in xrange(training_progress.eidx, max_epochs):
        ...
        for x, y in train:
            ...
            if model_options['objective'] == 'CE':
                ...
                for x_s, y_s in xy_pairs:
                    ...
                    """
                    mean_loss is used for normalization the BLEU score in different source sentences.
                    for example, sentence A's translation BLEU is high, e.g. 40 (all translation samples are similar values)
                        but sentence B's translation BLEU is low, e.g. 20
                        if we simply calculate, sentence A can cause larger loss than B for optimization
                        to smooth the optimization progress, we can use the mean value of every sentence to balance this bias
                    """
            ...
            if validFreq and training_progress.uidx >= validation_after_updates \
                    and numpy.mod(training_progress.uidx - validation_after_updates, validFreq) == 0:
                print 'beg validate'
                valid_start = time.time()  # add by lifuxue
                if early_stop_flag == "COST":
                    print 'beg cost valid'
                    use_noise.set_value(0.)
                ...
                if training_progress.uidx - validation_after_updates == 0 or valid_err <= numpy.array(training_progress.history_errs).min():
        ...
        print "Epoch %s time consume %.2f seconds" % (training_progress.eidx, epochCostTime)  # add by lifuxue
