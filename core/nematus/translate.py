...
def translate_model(queue, rqueue, pid, models, options, k, normalization_alpha, 
                    verbose, nbest, return_alignment, suppress_unk, 
                    return_hyp_graph, deviceid, return_identifier):
    ...
    ## when we translate sentences, we don't use noise !
    ...
    for model, option in zip(models, options):
        ...
        print 'return identifier --> ', return_identifier
        # word index
        f_init, f_next = build_sampler(tparams, option, use_noise, trng,
                                       return_alignment=return_alignment, return_identifier=return_identifier)
    ...
    def _translate(seq):
        ...
        sample, score, word_probs, alignment, identifier, hyp_graph = gen_sample(fs_init, fs_next,
                                   numpy.array(seq).T.reshape([len(seq[0]), len(seq), 1]),
                                   trng=trng, k=k, maxlen=200,
                                   stochastic=False, argmax=False, return_alignment=return_alignment,
                                    suppress_unk=suppress_unk, return_identifier=return_identifier,
        ...
        if nbest:
            return sample, score, word_probs, alignment, identifier, hyp_graph
        else:
            sidx = numpy.argmin(score)
            if return_identifier:
                return sample[sidx], score[sidx], word_probs[sidx], alignment[sidx], identifier[0], hyp_graph
            else:
                return sample[sidx], score[sidx], word_probs[sidx], alignment[sidx], None, hyp_graph
    ...
    while True:
        ...
        idx, x = req[0], req[1]
        #if verbose:
            #sys.stderr.write('{0} - {1}\r'.format(pid,idx))
        seq = _translate(x)
...
def print_replace_matrix(alignment ,i, target, pro , source, relpace_filename):
  d_json = {}
  d_json['id'] = i
  d_json['prob'] = pro
  d_json["target_sent"] = target
  d_json["source_sent"] = "%s <EOS>" % source # deal conditions for a target word aligns to <EOS> in source sentence
  my_list = []
  for it in alignment:
    tmp = it.tolist()
    my_list.append(tmp)
  d_json["matrix"] = my_list
  jsonStr = json.dumps(d_json)
  print >>relpace_filename, jsonStr
...
def main(models, source_file, saveto, save_alignment=None, k=5,
         normalization_alpha=0.0, n_process=5, chr_level=False, verbose=False, nbest=False, 
         suppress_unk=False, a_json=False, print_word_probabilities=False, 
         matrix_alignment=False, print_identifier=False,
         return_hyp_graph=False, device_list=[]):
    print 'print-identidier:{}'.format(args.print_identifier)
    ...
    for midx in xrange(n_process):
        ...
        processes[midx] = Process(
            ...
            args=(queue, rqueue, midx, models, options, k, normalization_alpha, 
            verbose, nbest, save_alignment is not None, 
            suppress_unk, return_hyp_graph, deviceid, print_identifier))
        ...
        for idx, line in enumerate(f):
            ...
            for w in words:
                ...
                if len(w) != options[0]['factors']:
                    ...
                    sys.exit(1)
                x.append(w)
            # add <EOS> at the end of input sequences
    ...
    def _retrieve_jobs(n_samples):
        ...
        for idx in xrange(n_samples):
            ...
            if verbose and numpy.mod(idx, 10) == 0:
                sys.stderr.write('Sample {0} / {1} Done\r'.format((idx+1), n_samples))
    ...
    if print_identifier:
        identifier_file = open(source_file.name+'.identidier', 'w')
    for i, trans in enumerate(_retrieve_jobs(n_samples)):
        if nbest:
            samples, scores, word_probs, alignment, identifier, hyp_graph = trans
            # draw decoding graph
            if return_hyp_graph:
            ...
            for j in order:
                if print_word_probabilities:
                    probs = " ||| " + " ".join("{0:.4f}".format(prob) for prob in word_probs[j])
                else:
                    probs = ""
                ...
                if save_alignment is not None:
                  if a_json:
                    ...
                    #TO do  add nbest matrix_alignment
        else:
            samples, scores, word_probs, alignment, identifier, hyp_graph = trans
            # draw decoding graph
            if return_hyp_graph:
            ...
            if save_alignment is not None:
                if a_json:
                    ...
                    if matrix_alignment:
                        print_replace_matrix(trans[3] ,i, _seqs2words(trans[0]), 0 , ' '.join(source_sentences[i]), matrix_alignment)

            if print_identifier:
                for prob in identifier:
                    print >>identifier_file, '{:.4f}'.format(prob),
    ...
    avg_length = numpy.array([len(s) for s in source_sentences]).mean()
    sys.stderr.write('Total {} sentences with average length {:.2f}, '
                     'using {:.2f} seconds, speed {:.2f} sent/s'.
                     format(n_samples, avg_length, used_time, float(n_samples)/used_time))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--matrix_alignment', '-ma', type=argparse.FileType('w'),
                        default=None, metavar='PATH',
                        help="Output file for alignment weights (default: standard output)")
    ...
    ## dump identifier
    parser.add_argument('--print_identifier', '-pi', action="store_true", help="Print identidier")
    ...
    main(args.models, args.input,
         ...
         matrix_alignment=args.matrix_alignment, print_identifier=args.print_identifier,
