...
class TextIterator:
    ...
    def __init__(self, source, target,
        ...
        """
            comment: only reserve 'n_words_source' words in dictionary
        """
    ...
    def next(self):
        ...
        # find a group of mini-batch for sort by target length
        if len(self.source_buffer) == 0:
            for ss in self.source:
                ss = ss.split()
                tt = self.target.readline().split()

                # pass empty line and too long line
            ...
            """
                comment: sort by length of target, using ascent order
           """
